...
=== `system-mysql` OR `system-postgres` OR `system-oracle`
...
`backend-redis`. `system-<_database_>` is the source of truth for this information.
...
[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-postgres' -o json | jq -r '.items[0].metadata.name') bash -c '_new stuff here_' | gzip > system-postgres-backup.gz
----
...
[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-oracle' -o json | jq -r '.items[0].metadata.name') bash -c '_new stuff here_' | gzip > system-oracle-backup.gz
----
...
Copy the PostgreSQL dump to the system-postgres pod

[source,bash]
----
oc cp ./system-postgres-backup.gz $(oc get pods -l 'deploymentConfig=system-postgres' -o json | jq '.items[0].metadata.name' -r):_???/var/lib/postgres???_
----
...
[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-postgres' -o json | jq -r '.items[0].metadata.name') bash -c 'gzip -d ${HOME}/system-postgres-backup.gz'
----
...
[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-postgres' -o json | jq -r '.items[0].metadata.name') bash -c '_new stuff here_'
----

==== `system-oracle`

Copy the Oracle dump to the system-oracle pod

[source,bash]
----
oc cp ./system-oracle-backup.gz $(oc get pods -l 'deploymentConfig=system-oracle' -o json | jq '.items[0].metadata.name' -r):_???/var/lib/oracle???_
----
...
[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-oracle' -o json | jq -r '.items[0].metadata.name') bash -c 'gzip -d ${HOME}/system-oracle-backup.gz'
----

Restore the Oracle DB Backup file

[source,bash]
----
oc rsh $(oc get pods -l 'deploymentConfig=system-oracle' -o json | jq -r '.items[0].metadata.name') bash -c '_new stuff here_'
----
